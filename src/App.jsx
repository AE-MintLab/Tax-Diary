...
export default function App() {
  ...
  const auditHealthScore = useMemo(() => {
    ...
    CATS.forEach(c => {
      ...
      if (sp === lim && sp > 0) {
        flags.push({ level: "info", msg: `${c.name} is claimed at exactly 100% cap (RM ${lim.toLocaleString(undefined, { maximumFractionDigits: 2 })}). Ensure full proof is attached.` });
      }
    });
  ...
  const spouseAnalysisDetailed = useMemo(() => {
    ...
    if (grossSpouse === 0) {
      ...
      reasoning = `Separate Assessment saves RM ${savings.toLocaleString(undefined, { maximumFractionDigits: 2 })}. Each of you keeps your own RM9,000 self-relief and starts fresh at the bottom of the progressive tax brackets.`;
    } else if (totalTaxJoint < totalTaxSeparate) {
      recommended = "JOINT";
      reasoning = `Joint Assessment saves RM ${savings.toLocaleString(undefined, { maximumFractionDigits: 2 })} for your household this year.`;
    } else {
      recommended = "EITHER";
      reasoning = `Both options result in the same tax liability of RM ${totalTaxSeparate.toLocaleString(undefined, { maximumFractionDigits: 2 })}.`;
    }
  ...
  const handleAmountChange = (val) => {
    ...
    if (max > 0 && num > max) { setForm(f => ({ ...f, amount: String(max) })); showToast(`Auto-capped to RM ${max.toLocaleString(undefined, { maximumFractionDigits: 2 })}`); }
...
<div className="bg-gradient-to-br from-[#42CFC0] via-[#8EDDB7] to-[#FFF0B5] rounded-3xl p-5 sm:p-6 shadow-xl relative overflow-hidden space-y-4">
  ...
  <div className="grid grid-cols-2 gap-3 relative z-30">
    ...
    <div className="bg-white/90 p-3 rounded-2xl shadow-sm">
      ...
      <p className="text-lg sm:text-2xl font-black mt-0.5 text-[#173B67]">RM {totalReliefs.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
      <p className="text-[10px] font-semibold text-[#58708F]">Chargeable income: RM {chargeable.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
    </div>
  </div>
</div>
        ...
        {taxYear === CURRENT_YA && (
          <div className={`bg-white p-4 rounded-3xl border shadow-sm space-y-2 ${daysLeft <= 30 ? "border-red-200" : daysLeft <= 90 ? "border-[#FF9F43]/40" : "border-gray-200"}`}>
            ...
            <div className="flex justify-between text-xs text-gray-500">
              ...
              <span className="font-bold text-[#20BFAE]">RM {opps.reduce((s, c) => s + c.rem, 0).toLocaleString(undefined, { maximumFractionDigits: 2 })} available</span>
            </div>
          </div>
        )}
                  ...
                  <div key={g} className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                    <button onClick={() => setExpanded(p => ({ ...p, [g]: !p[g] }))} className="w-full p-4 flex items-center justify-between bg-gray-50/80 hover:bg-gray-100 transition border-b border-gray-100">
                      <div className="flex items-center gap-2">
                        ...
                        {gS > 0 && <span className="text-xs px-2.5 py-0.5 rounded-full bg-[#E7F8F1] text-[#20BFAE] font-extrabold border border-[#20BFAE]/40">RM {gS.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>}
                      </div>
                      <div className="flex items-center gap-2 text-xs text-gray-400">
                        <span>Max RM {gL.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                            ...
                            <div key={c.id} className="p-4 space-y-2 hover:bg-gray-50/40 transition">
                              <div className="flex justify-between items-start">
                                ...
                                <div className="text-right text-xs">
                                  {spent > 0 ? <p className="font-extrabold text-[#20BFAE]">RM {spent.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p> : <p className="text-gray-400 font-medium">RM 0.00</p>}
                                  {isMaxed ? <span className="text-[#20BFAE] font-bold text-[10px] flex items-center gap-0.5 justify-end mt-0.5"><CheckCircle2 className="w-3 h-3" /> Maxed</span> : <span className="text-gray-400 text-[10px]">RM {rem.toLocaleString(undefined, { maximumFractionDigits: 2 })} remaining</span>}
                                </div>
                              </div>
                              ...
                              {!isMaxed && rem > 0 && (
                                <div className="flex items-center justify-between text-xs bg-[#FFF8D9] p-2 rounded-xl border border-[#F4D96B] gap-2">
                                  <span className="text-[11px] text-[#B8860B] font-medium flex items-center gap-1 flex-wrap">
                                    <Sparkles className="w-3 h-3 text-[#B8860B]" /> <strong>RM {rem.toLocaleString(undefined, { maximumFractionDigits: 2 })}</strong> unclaimed
                                    {isPro ? (
                                      <span className="text-[#B8860B] font-bold">· saves ~RM {estSave.toLocaleString(undefined, { maximumFractionDigits: 2 })} tax</span>
                                    ) : (
                  ...
                  {opps.slice(0, 4).map(c => (
                    <div key={c.id} className="p-3 rounded-2xl bg-gray-50 border border-gray-100 hover:border-[#F4D96B] transition space-y-1.5">
                      <div className="flex justify-between items-start text-xs">
                        ...
                        <span className="font-extrabold text-[#B8860B]">RM {c.rem.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                      </div>
                  ...
                  {activeR.map(r => (
                    <div key={r.id} className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-gray-50 border border-gray-100 text-xs">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-gray-400">{getCat(r.category)?.name || r.category} · RM {r.amount.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
                      </div>
                      {r.image ? (
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-[#EAF5FD] border border-[#A9D8F5] rounded-2xl space-y-2 text-xs text-[#1F5C82]">
                <div className="flex justify-between"><span>Employment Income:</span><span className="font-bold">RM {scenarioCalc.grossEmp.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                <div className="flex justify-between"><span>Net Side Business Profit:</span><span className="font-bold text-[#2E7CB8]">+ RM {scenarioCalc.netSide.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                <div className="flex justify-between"><span>Net Taxable Rental Income:</span><span className="font-bold text-[#2E7CB8]">+ RM {scenarioCalc.netRent.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                <div className="border-t border-[#A9D8F5] pt-2 flex justify-between font-black text-sm"><span>Combined Taxable Income:</span><span>RM {scenarioCalc.totalTaxable.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
              </div>
              ...
              {scenarioCalc.isConnected ? (
                <div className="p-3.5 bg-[#E7F8F1] border border-[#20BFAE]/40 rounded-2xl space-y-2">
                  ...
                  <p className="text-[11px] text-[#20BFAE]">Your dashboard, refund/owed, and Spouse tab all currently include this RM {scenarioCalc.netOther.toLocaleString(undefined, { maximumFractionDigits: 2 })} as real income.</p>
                ...
                <div className="p-3.5 bg-gray-50 border border-gray-200 rounded-2xl space-y-2">
                  ...
                  <button onClick={() => setOtherIncomeAmt(String(scenarioCalc.netOther))} disabled={scenarioCalc.netOther <= 0} className="w-full py-2.5 rounded-xl bg-[#4A90C2] hover:bg-[#3B7BAA] text-white font-bold text-xs disabled:opacity-40 disabled:cursor-not-allowed">Include RM {scenarioCalc.netOther.toLocaleString(undefined, { maximumFractionDigits: 2 })} in My Tax Calculation</button>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="p-5 space-y-5">
              <div>
                ...
                <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-between gap-2">
                  ...
                  <button onClick={() => { navigator.clipboard?.writeText(String(Math.round(parseFloat(income) || 0))); showToast(`C1 copied: RM ${(parseFloat(income) || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}`); }} className="shrink-0 flex flex-col items-end gap-1">
                    <span className="font-extrabold text-gray-900 text-xs">RM {(parseFloat(income) || 0).toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                    ...
                    .map(r => (
                      <div key={r.line} className={`p-3 rounded-2xl border flex items-center justify-between gap-2 ${r.val > 0 ? "bg-gray-50 border-gray-100" : "bg-white border-gray-100 opacity-70"}`}>
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-gray-400">Statutory Cap: RM {r.limit.toLocaleString(undefined, { maximumFractionDigits: 2 })}</p>
                        </div>
                        <button onClick={() => { navigator.clipboard?.writeText(String(r.val)); showToast(`${r.line} copied: RM ${r.val.toLocaleString(undefined, { maximumFractionDigits: 2 })}`); }} className="shrink-0 flex flex-col items-end gap-1">
                          <span className={`font-extrabold text-xs ${r.val > 0 ? "text-gray-900" : "text-gray-400"}`}>RM {r.val.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                          <span className="px-2.5 py-1 bg-white border border-gray-200 rounded-lg font-bold text-[10px] flex items-center gap-1 hover:border-[#20BFAE]"><ClipboardCopy className="w-3 h-3" /> Copy</span>
                        </button>
                      </div>
                    ))}
                    ...
                    return (
                      <div key={r.id} className="py-3 flex items-center justify-between text-xs hover:bg-gray-50/60 transition rounded-xl px-1">
                        ...
                        <div className="flex items-center gap-2 ml-2 shrink-0">
                          <span className="font-extrabold text-[#20BFAE] text-sm">RM {r.amount.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className={`rounded-2xl border overflow-hidden ${spouseAnalysisDetailed.recommended === "SEPARATE" ? "border-[#20BFAE] ring-1 ring-[#20BFAE]/40" : "border-gray-200 opacity-90"}`}>
                            ...
                            <table className="w-full text-[11px]">
                              ...
                              <tbody className="divide-y divide-gray-100">
                                <tr><td className="px-3 py-1.5 text-gray-600">Gross Income</td><td className="px-3 py-1.5 text-right font-semibold">RM {spouseAnalysisDetailed.separate.grossMine.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td><td className="px-3 py-1.5 text-right font-semibold">RM {spouseAnalysisDetailed.separate.grossSpouse.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td></tr>
                                <tr><td className="px-3 py-1.5 text-gray-600">Total Reliefs Claimed</td><td className="px-3 py-1.5 text-right text-[#20BFAE] font-semibold">-RM {spouseAnalysisDetailed.separate.selfReliefs.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td><td className="px-3 py-1.5 text-right text-[#20BFAE] font-semibold">-RM {spouseAnalysisDetailed.separate.spouseReliefs.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td></tr>
                                <tr><td className="px-3 py-1.5 text-gray-800 font-semibold">Chargeable Income</td><td className="px-3 py-1.5 text-right font-bold">RM {spouseAnalysisDetailed.separate.chargeableSelfSep.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td><td className="px-3 py-1.5 text-right font-bold">RM {spouseAnalysisDetailed.separate.chargeableSpouseSep.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td></tr>
                                <tr className="bg-gray-50"><td className="px-3 py-1.5 font-bold text-gray-900">Tax Payable</td><td className="px-3 py-1.5 text-right font-black">RM {spouseAnalysisDetailed.separate.taxSelfSep.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td><td className="px-3 py-1.5 text-right font-black">RM {spouseAnalysisDetailed.separate.taxSpouseSep.toLocaleString(undefined, { maximumFractionDigits: 2 })}</td></tr>
                              </tbody>
                            </table>
                            <div className="px-3 py-2 bg-white border-t border-gray-100 flex justify-between items-center">
                              ...
                              <span className="font-black text-[#20BFAE] text-sm">RM {spouseAnalysisDetailed.separate.totalTax.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                            </div>
                            <div className={`px-3 py-2 border-t flex justify-between items-center ${(spouseAnalysisDetailed.separate.balanceSelfSep + spouseAnalysisDetailed.separate.balanceSpouseSep) <= 0 ? "bg-[#E7F8F1] border-[#20BFAE]/20" : "bg-[#FFF3E8] border-[#FF9F43]/30"}`}>
                              ...
                              <span className="font-black text-sm">RM {Math.abs(spouseAnalysisDetailed.separate.balanceSelfSep + spouseAnalysisDetailed.separate.balanceSpouseSep).toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                            </div>
                          </div>
                          ...
                          <div className={`rounded-2xl border overflow-hidden ${spouseAnalysisDetailed.recommended === "JOINT" ? "border-[#20BFAE] ring-1 ring-[#20BFAE]/40" : "border-gray-200 opacity-90"}`}>
                            ...
                            <div className="p-3 space-y-1.5 text-[11px] bg-white">
                              <div className="flex justify-between"><span className="text-gray-600">Combined Gross Income:</span><span className="font-semibold">RM {spouseAnalysisDetailed.joint.combinedGross.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                              <div className="flex justify-between"><span className="text-gray-600">Combined Reliefs (incl. RM4,000 spouse relief{spouseDisabled ? " + RM6,000 disabled" : ""}):</span><span className="text-[#20BFAE] font-semibold">-RM {spouseAnalysisDetailed.joint.jointReliefTotal.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                              <div className="flex justify-between font-semibold text-gray-800 pt-1 border-t border-gray-100"><span>Chargeable Income:</span><span>RM {spouseAnalysisDetailed.joint.chargeableJoint.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                            </div>
                            <div className="px-3 py-2 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                              ...
                              <span className="font-black text-[#20BFAE] text-sm">RM {spouseAnalysisDetailed.joint.totalTax.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                            </div>
                            <div className={`px-3 py-2 border-t flex justify-between items-center ${spouseAnalysisDetailed.joint.balanceJoint <= 0 ? "bg-[#E7F8F1] border-[#20BFAE]/20" : "bg-[#FFF3E8] border-[#FF9F43]/30"}`}>
                              ...
                              <span className="font-black text-sm">RM {Math.abs(spouseAnalysisDetailed.joint.balanceJoint).toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                            </div>
                          </div>
                          ...
                          <div className="p-3 rounded-2xl bg-gray-50 border border-gray-200 space-y-3">
                            ...
                            {spouseAnalysisDetailed.catBreakdown.filter(c => c.mine > 0 || c.spouse > 0).map(c => (
                              <div key={c.id} className="space-y-1">
                                <div className="flex justify-between text-[11px] text-gray-700 font-semibold"><span className="truncate pr-2">{c.name}</span><span className="text-gray-400 font-normal shrink-0">Cap RM {c.limit.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-[9px] text-gray-500 w-16 text-right shrink-0">RM {c.mine.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-[9px] text-gray-500 w-16 text-right shrink-0">RM {c.spouse.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                                </div>
                              </div>
                            ))}
                ...
                <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100 space-y-3">
                  ...
                  {parseFloat(otherIncomeAmt) > 0 && (
                    <div className="flex items-center justify-between p-2.5 bg-[#EAF5FD] border border-[#A9D8F5] rounded-xl text-xs">
                      ...
                      <span className="font-bold text-[#1F5C82]">RM {parseFloat(otherIncomeAmt).toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                    </div>
                  )}
                ...
                {!hasSpouse ? (
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div><label className="font-bold text-gray-700 block mb-1">Your Income</label><div className="p-3 rounded-2xl bg-gray-100 font-extrabold text-sm border border-gray-200">RM {totalIncome.toLocaleString(undefined, { maximumFractionDigits: 2 })}</div></div>
                    ...
                    <div className="p-4 rounded-2xl bg-[#E7F8F1] border border-[#20BFAE]/40 space-y-3 text-xs">
                      ...
                      <div className="flex justify-between p-2 rounded-xl bg-white/60 border border-[#20BFAE]/20"><span className="text-gray-600">Joint Filing Tax:</span><span className="font-extrabold text-rose-600">RM {spouseAnalysisDetailed.joint.totalTax.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                      <div className="flex justify-between p-2 rounded-xl bg-white/60 border border-[#20BFAE]/20"><span className="text-gray-600">Separate Filing:</span><span className="font-extrabold text-[#20BFAE]">RM {spouseAnalysisDetailed.separate.totalTax.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span></div>
                      <div className="pt-2 border-t border-[#20BFAE]/40 flex items-start gap-2">
                        ...
                        <div><p className="font-extrabold text-sm text-[#0d9488]">{spouseAnalysisDetailed.recommended === "SEPARATE" ? "File Separately" : spouseAnalysisDetailed.recommended === "JOINT" ? "File Jointly" : "Either — same result"}</p><p className="text-[11px] text-[#20BFAE] font-normal">{spouseAnalysisDetailed.savings > 0 ? `Saves RM ${spouseAnalysisDetailed.savings.toLocaleString(undefined, { maximumFractionDigits: 2 })} in household taxes` : "Both options result in the same tax"}</p></div>
                      </div>
                    </div>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                ...
                <button onClick={() => { setShowReceipt(false); setShowCatPick(true); }} className="w-full p-3 rounded-xl border border-gray-200 bg-white font-semibold text-gray-800 flex items-center justify-between hover:bg-gray-50">
                  <span>{form.category ? `${getCat(form.category)?.name} (max RM ${getCatLimit(getCat(form.category)).toLocaleString(undefined, { maximumFractionDigits: 2 })})` : "Select category…"}</span>
                  <ChevronDown className="w-4 h-4 text-gray-400" />
                </button>
                {form.category && (() => { const { rem } = getStats(form.category); const orig = editId ? (receipts.find(r => r.id === editId)?.amount || 0) : 0; const max = rem + orig; return max > 0 ? <p className="text-[11px] text-[#20BFAE] mt-1">RM {max.toLocaleString(undefined, { maximumFractionDigits: 2 })} available to claim</p> : null; })()}
              ...
              <div key={g}>
                ...
                {CATS.filter(c => c.g === g).map(c => (
                  <button key={c.id} onClick={() => { setForm(f => ({ ...f, category: c.id, amount: "" })); setShowCatPick(false); setShowReceipt(true); }} className={`w-full text-left px-4 py-3 rounded-2xl text-xs font-bold flex items-center justify-between transition mb-1 ${form.category === c.id ? "bg-[#E7F8F1] text-[#0d9488] border border-[#20BFAE]/40" : "text-gray-700 hover:bg-gray-50"}`}>
                    <div><p className="font-bold">{c.emoji} {c.name}</p><p className="text-[10px] text-gray-400 font-normal">Max RM {getCatLimit(c).toLocaleString(undefined, { maximumFractionDigits: 2 })}</p></div>
