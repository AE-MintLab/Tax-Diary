...
const CATS = [
  ...
  { id: "socso",     g: "Financial", name: "SOCSO & EIS",                    nameBM: "SOCSO & EIS",                         limit: 350,   note: "Employee SOCSO / EIS contributions",                       beLine: "D11", emoji: "📋" },
...
const SLIDES = [
  { badge: "🗂️ SMART TAX VAULT",   headline: "STRESS-FREE\ne-FILING YA 2026", body: "Store LHDN receipts securely with full Budget 2026 updates (CCTV, Transit & Tourism). Be audit-ready for 7 years.", bg: "from-blue-950 via-indigo-900 to-gray-900" },
  { badge: "💡 SMART SUGGESTIONS", headline: "MAXIMIZE YOUR\nREFUNDS", body: "Tax Diary flags unclaimed YA 2026 opportunities and calculates exact ringgit tax savings in real-time.", bg: "from-indigo-950 via-gray-900 to-blue-950" },
  { badge: "👑 TRY VAULT FREE",    headline: "7 DAYS FREE\nTHEN RM19/YR", body: "Unlock AI receipt scanning, the Audit-Ready Checklist, Form BE sheet & multi-device cloud sync.", bg: "from-gray-950 via-blue-950 to-indigo-950" },
];
...
export default function App() {
  ...
  const init = async () => {
    try {
      ...
      if (i?.value) {
        ...
        setIncome(p.income || ""); setOtherIncomeAmt(p.otherIncomeAmt || "0"); setEpfAmt(p.epf || ""); setSocsoAmt(p.socsoAmt || ""); setZakatAmt(p.zakatAmt || ""); setIsSelfOKU(p.isSelfOKU || false);
  ...
  const buildCloudSnapshot = () => ({
    ...
    socsoAmt: sanitizeScalar(socsoAmt, ""),
    pcbAmt: sanitizeScalar(pcbAmt, ""),
    zakatAmt: sanitizeScalar(zakatAmt, ""),
    ...
    spouseSocsoAmt: sanitizeScalar(spouseSocsoAmt, ""),
      ...
      <div className={`fixed inset-0 bg-gradient-to-br ${sl.bg} flex flex-col justify-between overflow-hidden select-none`} onTouchStart={onTS} onTouchEnd={onTE}>
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center space-y-4">
            ...
            <p className="text-blue-300 text-xs font-bold tracking-widest uppercase">{sl.badge.slice(3)}</p>
        ...
        <div className="p-8 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex gap-2">{SLIDES.map((_, i) => (<button key={i} onClick={() => setSlideIdx(i)} className={`rounded-full transition-all ${slideIdx === i ? "w-8 h-2.5 bg-blue-400" : "w-2.5 h-2.5 bg-white/30"}`} />))}</div>
            <button onClick={() => slideIdx < SLIDES.length - 1 ? setSlideIdx(p => p + 1) : doneOnboard()} className="w-14 h-14 rounded-full bg-white text-gray-950 font-bold flex items-center justify-center shadow-xl hover:bg-blue-400 transition active:scale-95">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-slate-50 to-indigo-50 font-sans flex flex-col">
      {toast && (
        <div className="fixed bottom-5 right-5 z-[100] bg-gray-900 text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-2 text-xs font-semibold animate-bounce">
          <CheckCircle2 className="w-4 h-4 text-blue-400" />{toast}
        </div>
      )}
        ...
        <div className="bg-gray-900 text-white text-xs font-bold py-2.5 px-4 flex items-center justify-center gap-3 flex-wrap">
          ...
          {isIOS ? (
            ...
            <>
              ...
              <button onClick={handleInstallClick} className="px-3 py-1 bg-blue-600 hover:bg-blue-500 rounded-lg">Install</button>
            </>
          )}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">YA {taxYear}</span>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 space-y-4 pb-24">
        ...
        {/* Hero Banner */}
        <div className="bg-white/90 backdrop-blur-md rounded-3xl p-5 sm:p-6 shadow-xl border-2 border-blue-200/70 relative overflow-hidden space-y-4">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-blue-200/50 rounded-full blur-2xl pointer-events-none" />
          <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-indigo-200/50 rounded-full blur-2xl pointer-events-none" />

          <div className="border-b border-blue-100 pb-4 relative">
            <h1 className="text-xl sm:text-2xl font-black tracking-tight text-gray-800">Salam{clientName ? `, ${clientName}` : ""} 👋</h1>
            <p className="text-xs text-indigo-500/80 font-semibold">Track YA {taxYear} reliefs · Log receipts · Maximize your refund before 31 Dec {taxYear}</p>
          </div>
          <div className="grid grid-cols-2 gap-3 relative">
            <div className={`bg-gradient-to-br ${myBalance <= 0 ? "from-blue-50 to-rose-50 border-blue-200" : "from-amber-50 to-orange-50 border-amber-200"} border p-3 rounded-2xl`}>
              <p className={`text-[10px] font-black uppercase tracking-wider ${myBalance <= 0 ? "text-blue-600" : "text-amber-600"}`}>{myBalance <= 0 ? "Est. Refund" : "Est. Balance Owed"}</p>
              <p className={`text-lg sm:text-2xl font-black mt-0.5 ${myBalance <= 0 ? "text-blue-600" : "text-amber-600"}`}>{fmt(Math.abs(myBalance), 0)}</p>
              <p className={`text-[10px] font-medium opacity-70 ${myBalance <= 0 ? "text-blue-600" : "text-amber-600"}`}>vs PCB already paid</p>
            </div>
            <div className="bg-gradient-to-br from-indigo-50 to-indigo-50 border border-indigo-200 p-3 rounded-2xl">
              <p className="text-[10px] font-black uppercase tracking-wider text-indigo-600">Total Reliefs Claimed</p>
              <p className="text-lg sm:text-2xl font-black mt-0.5 text-indigo-600">RM {totalReliefs.toLocaleString()}</p>
              <p className="text-[10px] font-medium opacity-70 text-indigo-600">Chargeable income: RM {chargeable.toLocaleString()}</p>
            </div>
          </div>
        </div>
        ...
        {taxYear === 2026 && (
          <div className={`bg-white p-4 rounded-3xl border shadow-sm space-y-2 ${daysLeft <= 30 ? "border-red-200" : daysLeft <= 90 ? "border-orange-200" : "border-gray-200"}`}>
            <div className="flex justify-between text-xs font-bold">
              <span className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-blue-600" /> YA 2026 Planning Window</span>
            ...
            <div className="bg-gray-100 h-2.5 rounded-full overflow-hidden"><div className={`h-full rounded-full ${daysLeft <= 30 ? "bg-red-500" : daysLeft <= 90 ? "bg-orange-400" : "bg-blue-600"}`} style={{ width: `${yearPct}%` }} /></div>
            <div className="flex justify-between text-xs text-gray-500">
              <span>{opps.length} relief opportunities unfulfilled</span>
              <span className="font-bold text-blue-700">RM {opps.reduce((s, c) => s + c.rem, 0).toLocaleString()} available</span>
            </div>
          </div>
        )}

        {/* Quick Actions */}
        <button onClick={() => { setForm(blank(taxYear)); setEditId(null); setShowReceipt(true); }} className="w-full py-3.5 px-4 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm shadow flex items-center justify-center gap-2 active:scale-95 transition">
                  ...
                  <div key={g} className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
                    <button onClick={() => setExpanded(p => ({ ...p, [g]: !p[g] }))} className="w-full p-4 flex items-center justify-between bg-gray-50/80 hover:bg-gray-100 transition border-b border-gray-100">
                      <div className="flex items-center gap-2">
                        ...
                        {gS > 0 && <span className="text-xs px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-700 font-extrabold border border-blue-200">RM {gS.toLocaleString()}</span>}
                      </div>
                            ...
                            <div key={c.id} className="p-4 space-y-2 hover:bg-gray-50/40 transition">
                              <div className="flex justify-between items-start">
                                ...
                                <div className="text-right text-xs">
                                  {spent > 0 ? <p className="font-extrabold text-blue-700">RM {spent.toLocaleString()}</p> : <p className="text-gray-400 font-medium">RM 0.00</p>}
                              ...
                              <div className="bg-gray-100 h-2 rounded-full overflow-hidden">
                                <div className={`h-full rounded-full transition-all ${isMaxed ? "bg-emerald-500" : spent > 0 ? "bg-indigo-600" : "bg-transparent"}`} style={{ width: `${pct}%` }} />
                              </div>
                              {!isMaxed && rem > 0 && (
                                <div className="flex items-center justify-between text-xs bg-blue-50/60 p-2 rounded-xl border border-blue-100 gap-2">
                                  <span className="text-[11px] text-blue-900 font-medium flex items-center gap-1 flex-wrap">
                                    <Sparkles className="w-3 h-3 text-blue-600" /> <strong>RM {rem.toLocaleString()}</strong> unclaimed
                                    {isPro ? (
                                  ...
                                  {c.isAuto ? (
                                    <button onClick={() => openSettings("income")} className="text-[11px] font-bold text-blue-700 hover:underline shrink-0">Edit in Profile</button>
                                  ) : (
                                    <button onClick={() => { setForm({ ...blank(taxYear), category: c.id }); setShowReceipt(true); }} className="text-[11px] font-bold text-blue-700 hover:underline shrink-0">+ Add Receipt</button>
                                  )}
                                </div>
                              )}
                            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 pb-3">
                <h4 className="font-extrabold text-sm flex items-center gap-2"><Sparkles className="w-4 h-4 text-blue-600" /> Top Relief Opportunities</h4>
                <span className="text-[10px] bg-blue-50 text-blue-700 font-bold px-2 py-0.5 rounded-full border border-blue-200">YA {taxYear}</span>
              </div>
              {opps.length === 0 ? (
                <p className="text-xs text-blue-600 font-semibold text-center py-2">🎉 All categories have entries!</p>
              ) : (
                <div className="space-y-2.5">
                  {opps.slice(0, 4).map(c => (
                    <div key={c.id} className="p-3 rounded-2xl bg-gray-50 border border-gray-100 hover:border-blue-200 transition space-y-1.5">
                      <div className="flex justify-between items-start text-xs">
                        <span className="font-bold text-gray-800">{c.emoji} {c.name}</span>
                        <span className="font-extrabold text-blue-700">RM {c.rem.toLocaleString()}</span>
                      </div>
                      <p className="text-[11px] text-gray-500">{c.note}</p>
                      {c.isAuto ? (
                        <button onClick={() => openSettings("income")} className="w-full py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-800 font-bold text-[11px] border border-blue-200 flex items-center justify-center gap-1">Edit in Profile</button>
                      ) : (
                        <button onClick={() => { setForm({ ...blank(taxYear), category: c.id }); setShowReceipt(true); }} className="w-full py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-800 font-bold text-[11px] border border-blue-200 flex items-center justify-center gap-1"><Plus className="w-3 h-3" /> Add Receipt</button>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {(() => {
        ...
        const tabBtn = (active, onClick, Icon, label) => (
          <button onClick={onClick} className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-2xl transition ${active ? "text-blue-600" : "text-gray-400"}`}>
        ...
        <div className="fixed inset-0 z-[65] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => setShowTools(false)}>
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3">
              <div>
                <h3 className="font-extrabold text-base flex items-center gap-2"><Sliders className="w-5 h-5 text-blue-600" /> Tools</h3>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => { setShowTools(false); isPro ? setShowAuditCheck(true) : openPaywall("Audit-Ready Checklist", "A quick self-check so your claims are well-documented before you file.", () => setShowTools(true)); }} className="p-3 rounded-2xl bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-800 font-bold text-xs flex flex-col items-center gap-1.5 text-center">
                {isPro ? <ShieldCheck className="w-5 h-5 text-blue-600" /> : <Lock className="w-4 h-4 text-amber-500" />} Checklist
              </button>
              ...
              <button onClick={() => { setShowTools(false); isPro ? setShowFormBE(true) : openPaywall("LHDN Form BE Copy Sheet", "Get your relief totals mapped straight to Form BE lines D1–D18, ready to paste into MyTax.", () => setShowTools(true)); }} className="p-3 rounded-2xl bg-gray-50 hover:bg-gray-100 border border-gray-200 text-gray-800 font-bold text-xs flex flex-col items-center gap-1.5 text-center">
                {isPro ? <FileText className="w-5 h-5 text-blue-600" /> : <Lock className="w-4 h-4 text-amber-500" />} Form BE Sheet
              </button>
            ...
            <div className="bg-gradient-to-br from-gray-900 to-blue-950 rounded-3xl p-5 text-white shadow-md space-y-3">
              ...
              <a href="https://mytax.hasil.gov.my" target="_blank" rel="noreferrer" className="w-full py-2 bg-blue-700 hover:bg-blue-600 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition"><ExternalLink className="w-3.5 h-3.5" /> MyTax Official Portal</a>
            </div>
      ...
      {showAuditCheck && (
        <div className="fixed inset-0 z-[70] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => { setShowAuditCheck(false); setShowTools(true); }}>
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3">
              <h3 className="font-bold text-base flex items-center gap-2"><ShieldCheck className="w-5 h-5 text-blue-600" /> Audit-Ready Checklist</h3>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 text-center space-y-1">
              <p className="text-xs font-bold text-blue-800">Documentation Score</p>
              <p className="text-3xl font-black text-blue-700">{auditHealthScore.score}%</p>
              <p className="text-[11px] text-blue-600">{auditHealthScore.score >= 90 ? "Nicely done — your claims are well documented." : "A few things worth tidying up before you file."}</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs font-bold text-gray-700">Worth double-checking:</p>
              {auditHealthScore.flags.length === 0 ? (
                <p className="text-xs text-blue-600 font-semibold p-3 bg-gray-50 rounded-xl">✓ All claims have attached receipt images and reasonable claim amounts.</p>
              ) : (
            ...
            <button onClick={exportPDF} className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-extrabold rounded-2xl text-xs flex items-center justify-center gap-2 shadow">
        ...
        <div className="fixed inset-0 z-[70] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => { setShowScenario(false); setShowTools(true); }}>
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-gray-50 rounded-2xl border border-gray-200 space-y-2">
                <p className="font-bold text-xs text-gray-800 flex items-center gap-1.5"><Home className="w-4 h-4 text-blue-600" /> Rental Property Income</p>
                <div className="grid grid-cols-2 gap-2 text-xs">
              ...
              {scenarioCalc.isConnected ? (
                <div className="p-3.5 bg-blue-50 border border-blue-200 rounded-2xl space-y-2">
                  <p className="text-xs font-bold text-blue-800 flex items-center gap-1.5"><CheckCircle2 className="w-4 h-4" /> Included in your tax calculation</p>
                  <p className="text-[11px] text-blue-700">Your dashboard, refund/owed, and Spouse tab all currently include this RM {scenarioCalc.netOther.toLocaleString()} as real income.</p>
                  <button onClick={() => setOtherIncomeAmt("0")} className="w-full py-2 rounded-xl bg-white border border-blue-300 text-blue-700 font-bold text-xs">Remove From My Tax Calculation</button>
                </div>
              ) : (
        ...
        <div className="fixed inset-0 z-[90] bg-gray-900/70 backdrop-blur-sm overflow-y-auto p-4" onClick={closePaywall}>
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 space-y-4">
                ...
                <div className="space-y-3 text-xs pt-1">
                  {[
                    ...
                    <div key={group} className="space-y-1">
                      ...
                      {items.map(f => (
                        <div key={f} className="flex items-center gap-2 text-gray-200 pl-1"><Check className="w-3.5 h-3.5 text-blue-400 shrink-0" /> {f}</div>
                      ))}
                    </div>
                  ))}
                </div>
              ...
              {!trialUsed ? (
                <div className="space-y-2">
                  <button onClick={startTrial} disabled={paymentLoading} className="w-full py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-extrabold transition shadow-md">Start 7-Day Free Trial</button>
                ...
                <button onClick={subscribe} disabled={paymentLoading} className="w-full py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-extrabold transition shadow-md">{paymentLoading ? "Redirecting to payment…" : `Subscribe to Plus — RM${PRICE.toFixed(2)}/yr`}</button>
              )}
              {isTrialing ? (
        ...
        <div className="fixed inset-0 z-[70] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => { setShowFormBE(false); setShowTools(true); }}>
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl mx-auto my-8 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50">
              <div className="flex items-center gap-2.5">
                <div className="p-2.5 bg-blue-100 text-blue-700 rounded-2xl shrink-0"><Sparkles className="w-5 h-5" /></div>
                <div>
              ...
              <div>
                ...
                <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-between gap-2">
                  ...
                  <button onClick={() => { navigator.clipboard?.writeText(String(Math.round(parseFloat(income) || 0))); showToast(`C1 copied: RM ${(parseFloat(income) || 0).toLocaleString()}`); }} className="shrink-0 flex flex-col items-end gap-1">
                    ...
                    <span className="px-2.5 py-1 bg-white border border-gray-200 rounded-lg font-bold text-[10px] flex items-center gap-1 hover:border-blue-400"><ClipboardCopy className="w-3 h-3" /> Copy</span>
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-white rounded-3xl p-5 border border-gray-200 shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-gray-100 pb-3">
                <div>
                  <h3 className="font-bold text-base flex items-center gap-2"><Receipt className="w-5 h-5 text-blue-600" /> YA {taxYear} Receipt Vault</h3>
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1">
                {AUDIT_YRS.map(y => {
                  ...
                  return (
                    <button key={y} onClick={() => locked ? openPaywall("7-Year Vault Access", "Free plan only covers the current tax year. Upgrade to view and export past years.", () => openVault()) : setTaxYear(y)} className={`shrink-0 p-2 rounded-xl border text-center transition relative min-w-[64px] ${taxYear === y ? "bg-blue-600 text-white border-blue-700 font-extrabold" : locked ? "bg-gray-50 text-gray-400 border-gray-200" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-100 font-bold"}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <div className="relative flex-1">
                  ...
                  <input value={rcptSearch} onChange={e => setRcptSearch(e.target.value)} placeholder="Search vendor…" className="pl-8 pr-3 py-2 rounded-xl border border-gray-200 text-xs w-full outline-none focus:ring-2 focus:ring-blue-400" />
                </div>
              ...
              {filteredR.length === 0 ? (
                <div className="p-8 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200 space-y-2">
                  ...
                  <div className="flex gap-2 justify-center pt-1">
                    <button onClick={() => { setForm(blank(taxYear)); setShowVault(false); openReceiptModal(() => setShowVault(true)); }} className="px-4 py-2 bg-blue-600 text-white rounded-xl text-xs font-bold inline-flex items-center gap-1"><Plus className="w-3.5 h-3.5" /> Add</button>
                    <button onClick={() => isPro ? (() => { setShowVault(false); setReceiptReturnTo(() => () => setShowVault(true)); openScan(() => setShowReceipt(true)); })() : (() => { setShowVault(false); openPaywall("AI Receipt Scanner", "Snap a receipt and let AI fill in the details for you.", () => setShowVault(true)); })()} className="px-4 py-2 bg-gray-800 text-white rounded-xl text-xs font-bold inline-flex items-center gap-1">{isPro ? <Zap className="w-3.5 h-3.5 text-blue-400" /> : <Lock className="w-3.5 h-3.5 text-amber-400" />} AI Scan</button>
                  </div>
                </div>
              ) : (
                <div className="divide-y divide-gray-100">
                    ...
                    return (
                      <div key={r.id} className="py-3 flex items-center justify-between text-xs hover:bg-gray-50/60 transition rounded-xl px-1">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          ) : <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-700 border border-blue-100 flex items-center justify-center shrink-0"><Receipt className="w-5 h-5" /></div>}
                          <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 ml-2 shrink-0">
                          <span className="font-extrabold text-blue-700 text-sm">RM {r.amount.toLocaleString()}</span>
                          <button onClick={() => startEdit(r)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-600"><Edit2 className="w-3.5 h-3.5" /></button>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            {!hasSpouse ? (
              <div className="p-4 bg-gray-50 rounded-2xl border border-gray-200 text-xs text-gray-500 space-y-2">
                ...
                <button onClick={() => { setShowSpouseDetail(false); setMaritalStatus("married"); openSettings("user", () => setShowSpouseDetail(true)); }} className="text-blue-700 font-bold underline">Set marital status to Married in Profile</button>
              </div>
            ) : !isPro ? (
              <button onClick={() => { setShowSpouseDetail(false); openPaywall("Detailed Household Assessment", "See a full category-by-category breakdown of who should claim what, not just the aggregate number.", () => setShowSpouseDetail(true)); }} className="w-full py-2.5 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 font-bold text-xs border border-amber-200 flex items-center justify-center gap-1.5"><Lock className="w-3.5 h-3.5" /> Unlock Detailed Analysis</button>
                        ...
                        <div className="space-y-3">
                          <div className={`p-3 rounded-2xl border ${spouseAnalysisDetailed.recommended === "SEPARATE" ? "bg-blue-50 border-blue-300" : "bg-rose-50 border-rose-200"}`}>
                          ...
                          <div className={`rounded-2xl border overflow-hidden ${spouseAnalysisDetailed.recommended === "SEPARATE" ? "border-blue-300 ring-1 ring-blue-200" : "border-gray-200 opacity-90"}`}>
                            <div className="px-3 py-2 bg-gray-900 text-white flex items-center justify-between">
                              ...
                              {spouseAnalysisDetailed.recommended === "SEPARATE" && <span className="text-[9px] bg-blue-500 px-2 py-0.5 rounded-full font-bold">Recommended</span>}
                            </div>
                            <table className="w-full text-[11px]">
                              ...
                              <tbody className="divide-y divide-gray-100">
                                ...
                                <tr><td className="px-3 py-1.5 text-gray-600">Total Reliefs Claimed</td><td className="px-3 py-1.5 text-right text-blue-700 font-semibold">-RM {spouseAnalysisDetailed.separate.selfReliefs.toLocaleString()}</td><td className="px-3 py-1.5 text-right text-blue-700 font-semibold">-RM {spouseAnalysisDetailed.separate.spouseReliefs.toLocaleString()}</td></tr>
                            ...
                            <div className="px-3 py-2 bg-white border-t border-gray-100 flex justify-between items-center">
                              ...
                              <span className="font-black text-blue-700 text-sm">RM {spouseAnalysisDetailed.separate.totalTax.toLocaleString()}</span>
                            </div>
                            <div className={`px-3 py-2 border-t flex justify-between items-center ${(spouseAnalysisDetailed.separate.balanceSelfSep + spouseAnalysisDetailed.separate.balanceSpouseSep) <= 0 ? "bg-blue-50 border-blue-100" : "bg-amber-50 border-amber-100"}`}>
                          ...
                          {/* Joint Assessment */}
                          <div className={`rounded-2xl border overflow-hidden ${spouseAnalysisDetailed.recommended === "JOINT" ? "border-blue-300 ring-1 ring-blue-200" : "border-gray-200 opacity-90"}`}>
                            <div className="px-3 py-2 bg-gray-900 text-white flex items-center justify-between">
                              <span className="font-bold text-xs">Option B — Joint Assessment</span>
                              {spouseAnalysisDetailed.recommended === "JOINT" && <span className="text-[9px] bg-blue-500 px-2 py-0.5 rounded-full font-bold">Recommended</span>}
                            </div>
                            <div className="p-3 space-y-1.5 text-[11px] bg-white">
                              ...
                              <div className="flex justify-between"><span className="text-gray-600">Combined Reliefs (incl. RM4,000 spouse relief{spouseDisabled ? " + RM6,000 disabled" : ""}):</span><span className="text-blue-700 font-semibold">-RM {spouseAnalysisDetailed.joint.jointReliefTotal.toLocaleString()}</span></div>
                            ...
                            <div className="px-3 py-2 bg-gray-50 border-t border-gray-100 flex justify-between items-center">
                              ...
                              <span className="font-black text-blue-700 text-sm">RM {spouseAnalysisDetailed.joint.totalTax.toLocaleString()}</span>
                            </div>
                            <div className={`px-3 py-2 border-t flex justify-between items-center ${spouseAnalysisDetailed.joint.balanceJoint <= 0 ? "bg-blue-50 border-blue-100" : "bg-amber-50 border-amber-100"}`}>
                          ...
                          <div className="p-3 rounded-2xl bg-gray-50 border border-gray-200 space-y-3">
                            ...
                            {spouseAnalysisDetailed.catBreakdown.filter(c => c.mine > 0 || c.spouse > 0).map(c => (
                              <div key={c.id} className="space-y-1">
                                ...
                                <div className="flex items-center gap-1.5">
                                  <span className="text-[9px] text-blue-700 font-bold w-8 shrink-0">You</span>
                                  <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-blue-500" style={{ width: `${Math.min(100, (c.mine / c.limit) * 100)}%` }} /></div>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[60] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={closeSettings}>
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3">
              <div>
                <h3 className="font-extrabold text-base flex items-center gap-2"><Settings className="w-5 h-5 text-blue-600" /> Profile</h3>
            ...
            <div className="grid grid-cols-4 bg-gray-100 p-1 rounded-2xl border border-gray-200 text-[11px] font-bold gap-1">
              {[["user", "👤 Profile"], ["income", "🧮 Income"], ["spouse", "❤️ Spouse"], ["sync", "📱 Devices"]].map(([tab, label]) => (
                <button key={tab} onClick={() => setSettingsTab(tab)} className={`py-2 rounded-xl transition ${settingsTab === tab ? "bg-white text-blue-700 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}>{label}</button>
              ))}
            </div>
            ...
            {settingsTab === "user" && (
              <div className="space-y-3 text-xs">
                <div>
                  ...
                  <input type="text" value={clientName} onChange={e => setClientName(e.target.value)} placeholder="e.g. Ahmad Ridzuan" className="w-full p-3 rounded-xl border border-gray-200 font-semibold outline-none focus:ring-2 focus:ring-blue-400" />
                </div>
                <div className="p-4 bg-gray-50 rounded-2xl border border-gray-200 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button onClick={() => setMaritalStatus("single")} className={`py-2.5 rounded-xl border font-bold text-xs ${maritalStatus === "single" ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-500"}`}>Single</button>
                    <button onClick={() => setMaritalStatus("married")} className={`py-2.5 rounded-xl border font-bold text-xs ${maritalStatus === "married" ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-500"}`}>Married</button>
                  </div>
                </div>
                <div className="p-4 bg-gray-50 rounded-2xl border border-gray-200 flex items-center justify-between">
                  ...
                  <button onClick={() => setIsSelfOKU(!isSelfOKU)} className={`w-12 h-6 rounded-full flex items-center p-1 transition-colors shrink-0 ml-3 ${isSelfOKU ? "bg-blue-600 justify-end" : "bg-gray-300 justify-start"}`}><span className="w-4 h-4 bg-white rounded-full shadow" /></button>
                </div>
              </div>
            )}
            ...
            {settingsTab === "income" && (
              <div className="space-y-4">
                <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100 space-y-3">
                  ...
                  <div><label className="text-xs font-bold text-gray-700 block mb-1">Employment Income — All Employers Combined (RM)</label><input type="number" value={income} onChange={e => setIncome(e.target.value)} placeholder="e.g. 91800" className="w-full p-3 rounded-xl border border-gray-200 font-bold text-sm outline-none focus:ring-2 focus:ring-blue-400 bg-white" /><p className="text-[10px] text-gray-400 mt-1">If you had more than one job this year, enter the total across all of them</p></div>
                  ...
                  <div><label className="text-xs font-bold text-gray-700 block mb-1">Annual EPF Contribution (RM)</label><input type="number" value={epfAmt} onChange={e => setEpfAmt(e.target.value)} placeholder="e.g. 10098" className="w-full p-3 rounded-xl border border-gray-200 font-bold text-sm outline-none focus:ring-2 focus:ring-blue-400 bg-white" /><p className="text-[10px] text-gray-400 mt-1">LHDN relief cap: RM 4,000</p></div>
                  <div><label className="text-xs font-bold text-gray-700 block mb-1">Annual SOCSO / EIS Contribution (RM)</label><input type="number" value={socsoAmt} onChange={e => setSocsoAmt(e.target.value)} placeholder="e.g. 350" className="w-full p-3 rounded-xl border border-gray-200 font-bold text-sm outline-none focus:ring-2 focus:ring-blue-400 bg-white" /><p className="text-[10px] text-gray-400 mt-1">Auto-defaulted at the RM350 cap — a payroll deduction, no receipt needed</p></div>
                  <div><label className="text-xs font-bold text-gray-700 block mb-1">Total PCB Deducted This Year (RM)</label><input type="number" value={pcbAmt} onChange={e => setPcbAmt(e.target.value)} placeholder="e.g. 3600" className="w-full p-3 rounded-xl border border-gray-200 font-bold text-sm outline-none focus:ring-2 focus:ring-blue-400 bg-white" /><p className="text-[10px] text-gray-400 mt-1">The full-year total from your EA Form (Item E) — not a monthly figure</p></div>
                  <div><label className="text-xs font-bold text-gray-700 block mb-1">Zakat Paid This Year (RM)</label><input type="number" value={zakatAmt} onChange={e => setZakatAmt(e.target.value)} placeholder="e.g. 2400" className="w-full p-3 rounded-xl border border-gray-200 font-bold text-sm outline-none focus:ring-2 focus:ring-blue-400 bg-white" /><p className="text-[10px] text-gray-400 mt-1">Offsets your tax payable directly, RM-for-RM (not a relief on chargeable income)</p></div>
                  <div>
                    <label className="text-xs font-bold text-gray-700 block mb-1">First Home Price (for Home Loan Interest relief)</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button onClick={() => setHomeLoanTier("under500k")} className={`py-2 rounded-xl border font-bold text-[11px] ${homeLoanTier === "under500k" ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-500"}`}>≤ RM500k (cap RM7,000)</button>
                      <button onClick={() => setHomeLoanTier("500to750k")} className={`py-2 rounded-xl border font-bold text-[11px] ${homeLoanTier === "500to750k" ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-500"}`}>RM500k–750k (cap RM5,000)</button>
                    </div>
                ...
                <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100 space-y-4">
                  ...
                  {hasSpouse && (
                    <div className="flex items-center justify-between text-xs pl-3 border-l-2 border-blue-100">
                      ...
                      <button onClick={() => setSpouseDisabled(!spouseDisabled)} className={`w-12 h-6 rounded-full flex items-center p-1 transition-colors ${spouseDisabled ? "bg-blue-600 justify-end" : "bg-gray-300 justify-start"}`}><span className="w-4 h-4 bg-white rounded-full shadow" /></button>
                    </div>
                  )}
                  {[["Children under 18", "RM 2,000 each", childU18, setChildU18], ["Children Higher Edu (Diploma+/Degree/Masters/PhD)", "RM 8,000 each", childHiEduDegree, setChildHiEduDegree], ["Children Higher Edu (other full-time study)", "RM 2,000 each", childHiEduOther, setChildHiEduOther], ["Disabled Child", "RM 8,000 each", childDisabled, setChildDisabled], ["Disabled Child in Higher Edu", "+RM 8,000 more each", childDisabledHiEdu, setChildDisabledHiEdu]].map(([l, s, v, sv]) => (
                    <div key={l} className="flex items-center justify-between text-xs">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => sv(v + 1)} className="w-8 h-8 rounded-full bg-blue-100 text-blue-800 font-bold flex items-center justify-center">+</button>
                      </div>
                    </div>
                  ))}
                  ...
                  {hasSpouse && (childU18 + childHiEduDegree + childHiEduOther + childDisabled + childDisabledHiEdu > 0) && (
                    <div className="pl-3 border-l-2 border-blue-100 space-y-1.5">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => setChildrenClaimedBy("mine")} className={`py-2 rounded-xl border font-bold text-[11px] ${childrenClaimedBy === "mine" ? "bg-blue-50 border-blue-300 text-blue-700" : "bg-white border-gray-200 text-gray-500"}`}>You</button>
                ...
                <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 space-y-2 text-xs">
                  ...
                  <div className="flex justify-between font-bold text-blue-700"><span>Tax payable (with reliefs):</span><span>{fmt(taxAfter)}</span></div>
                  ...
                  <div className="p-3 bg-blue-100 rounded-xl flex justify-between items-center mt-1">
                    <span className="font-extrabold text-xs text-blue-900 tracking-wider">YOU'RE SAVING</span>
                    <span className="font-extrabold text-xl text-blue-900">{fmt(taxSaved)}</span>
                  </div>
                  <div className={`p-3 rounded-xl flex justify-between items-center ${myBalance <= 0 ? "bg-white border border-blue-200" : "bg-amber-50 border border-amber-200"}`}>
                    <span className={`font-extrabold text-xs tracking-wider ${myBalance <= 0 ? "text-blue-900" : "text-amber-900"}`}>{myBalance <= 0 ? "ESTIMATED REFUND" : "ESTIMATED BALANCE OWED"}</span>
                    <span className={`font-extrabold text-xl ${myBalance <= 0 ? "text-blue-900" : "text-amber-900"}`}>{fmt(Math.abs(myBalance))}</span>
                  </div>
            ...
            {settingsTab === "spouse" && (
              <div className="space-y-4">
                {!hasSpouse ? (
                  <div className="p-4 bg-gray-50 rounded-2xl border border-gray-200 text-xs text-gray-500 space-y-2">
                    <p>Spouse tools apply once you're set as married.</p>
                    <button onClick={() => { setMaritalStatus("married"); setSettingsTab("user"); }} className="text-blue-700 font-bold underline">Set marital status to Married in Profile</button>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 space-y-3 text-xs">
                      <p className="font-bold text-blue-900 text-sm">Quick Comparison:</p>
                      <div className="flex justify-between p-2 rounded-xl bg-white/60 border border-blue-100"><span className="text-gray-600">Joint Filing Tax:</span><span className="font-extrabold text-rose-600">RM {spouseAnalysisDetailed.joint.totalTax.toLocaleString()}</span></div>
                      <div className="flex justify-between p-2 rounded-xl bg-white/60 border border-blue-100"><span className="text-gray-600">Separate Filing:</span><span className="font-extrabold text-blue-700">RM {spouseAnalysisDetailed.separate.totalTax.toLocaleString()}</span></div>
                      <div className="pt-2 border-t border-blue-200 flex items-start gap-2">
                        <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
                        <div><p className="font-extrabold text-sm text-blue-800">{spouseAnalysisDetailed.recommended === "SEPARATE" ? "File Separately" : spouseAnalysisDetailed.recommended === "JOINT" ? "File Jointly" : "Either — same result"}</p><p className="text-[11px] text-blue-700 font-normal">{spouseAnalysisDetailed.savings > 0 ? `Saves RM ${spouseAnalysisDetailed.savings.toLocaleString()} in household taxes` : "Both options result in the same tax"}</p></div>
                      </div>
                    </div>

                    <div className="border-t border-gray-100 pt-4">
                      <button onClick={() => { setShowSettings(false); setShowSpouseDetail(true); }} className="w-full py-2.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 font-bold text-xs border border-blue-200 flex items-center justify-center gap-1.5">
                  ...
                  <div className="space-y-3">
                    ...
                    {!isPro ? (
                      ...
                      <div className="p-4 bg-blue-50 rounded-2xl border border-blue-200 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="font-extrabold text-sm text-blue-900 flex items-center gap-1.5"><Smartphone className="w-4 h-4" /> Cloud Sync</span>
                        </div>
            ...
            <button onClick={saveAll} className="w-full py-3.5 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white font-extrabold transition shadow-md">Save Settings</button>
          </div>
        </div>
      )}
      ...
      {showScan && (
        <div className="fixed inset-0 z-[60] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={closeScan}>
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3">
              <h3 className="font-bold text-base flex items-center gap-2"><Zap className="w-5 h-5 text-blue-600" /> AI Receipt Scanner (Budget 2026 Ready)</h3>
              <button onClick={closeScan}><X className="w-5 h-5 text-gray-400" /></button>
            </div>
            {ocrLoading ? (
              <div className="p-10 text-center space-y-4">
                <div className="w-16 h-16 rounded-full bg-blue-100 flex items-center justify-center mx-auto animate-pulse"><Sparkles className="w-8 h-8 text-blue-600" /></div>
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-blue-200 rounded-2xl p-6 text-center space-y-3 bg-blue-50">
                  ...
                  <label className="cursor-pointer inline-block">
                    <div className="px-6 py-2.5 rounded-xl bg-blue-600 text-white font-bold text-xs hover:bg-blue-500 transition inline-flex items-center gap-2"><Upload className="w-4 h-4" /> Choose Receipt Photo</div>
      ...
      {showReceipt && (
        <div className="fixed inset-0 z-[60] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={closeReceiptModal}>
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3">
              <h3 className="font-bold text-base flex items-center gap-2"><Plus className="w-4 h-4 text-blue-600" />{editId ? "Edit Receipt" : `Add Receipt (YA ${form.taxYear || taxYear})`}</h3>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                ...
                {form.category && (() => { const { rem } = getStats(form.category); const orig = editId ? (receipts.find(r => r.id === editId)?.amount || 0) : 0; const max = rem + orig; return max > 0 ? <p className="text-[11px] text-blue-600 mt-1">RM {max.toLocaleString()} available to claim</p> : null; })()}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="font-bold text-gray-700 block mb-1">Date</label><input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className="w-full p-2.5 rounded-xl border border-gray-200 font-semibold outline-none focus:ring-2 focus:ring-blue-400" /></div>
                <div><label className="font-bold text-gray-700 block mb-1">Amount (RM) *</label><input type="number" value={form.amount} onChange={e => handleAmountChange(e.target.value)} placeholder="0.00" className="w-full p-2.5 rounded-xl border border-gray-200 font-bold outline-none focus:ring-2 focus:ring-blue-400" /></div>
              </div>
              {hasSpouse && isPro && (
                <div>
              ...
              <div>
                ...
                <label className="cursor-pointer block">
                  <div className="w-full p-3 border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center gap-2 bg-gray-50 hover:border-blue-400 transition"><Upload className="w-4 h-4 text-gray-400" /><span className="text-gray-500 text-xs">Tap to attach receipt image</span></div>
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={editId ? handleUpdate : handleSave} className="flex-1 py-2.5 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-500">{editId ? "Update" : "Vault Receipt"}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showCatPick && (
        <div className="fixed inset-0 z-[70] bg-gray-900/70 backdrop-blur-sm overflow-y-auto" onClick={() => { setShowCatPick(false); setShowReceipt(true); }}>
          <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-md w-full p-5 shadow-2xl space-y-2 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3 sticky top-0 bg-white"><h3 className="font-bold text-base flex items-center gap-2"><Tag className="w-4 h-4 text-blue-600" /> Select LHDN Category</h3><button onClick={() => { setShowCatPick(false); setShowReceipt(true); }}><X className="w-5 h-5 text-gray-400" /></button></div>
            {GROUPS.map(g => (
              <div key={g}>
                <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider px-2 py-1">{g}</p>
                {CATS.filter(c => c.g === g).map(c => (
                  <button key={c.id} onClick={() => { setForm(f => ({ ...f, category: c.id, amount: "" })); setShowCatPick(false); setShowReceipt(true); }} className={`w-full text-left px-4 py-3 rounded-2xl text-xs font-bold flex items-center justify-between transition mb-1 ${form.category === c.id ? "bg-blue-50 text-blue-800 border border-blue-200" : "text-gray-700 hover:bg-gray-50"}`}>
                    ...
                    {form.category === c.id && <Check className="w-4 h-4 text-blue-600" />}
                  </button>
                ))}
              </div>
            ))}
      ...
      {showFilterPick && (
        <div className="fixed inset-0 z-[75] bg-gray-900/70 backdrop-blur-sm overflow-y-auto" onClick={() => { setShowFilterPick(false); setShowVault(true); }}>
          <div className="bg-white rounded-t-3xl sm:rounded-3xl max-w-md w-full p-5 shadow-2xl space-y-2 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center border-b border-gray-100 pb-3 sticky top-0 bg-white"><h3 className="font-bold text-base flex items-center gap-2"><Tag className="w-4 h-4 text-blue-600" /> Filter by Category</h3><button onClick={() => { setShowFilterPick(false); setShowVault(true); }}><X className="w-5 h-5 text-gray-400" /></button></div>
            <button onClick={() => { setRcptCatF("all"); setShowFilterPick(false); setShowVault(true); }} className={`w-full text-left px-4 py-3 rounded-2xl text-xs font-bold flex items-center justify-between transition mb-1 ${rcptCatF === "all" ? "bg-blue-50 text-blue-800 border border-blue-200" : "text-gray-700 hover:bg-gray-50"}`}>
              <span>All Categories</span>
              {rcptCatF === "all" && <Check className="w-4 h-4 text-blue-600" />}
            </button>
            {GROUPS.map(g => (
              <div key={g}>
                <p className="text-[10px] font-extrabold text-gray-400 uppercase tracking-wider px-2 py-1">{g}</p>
                {CATS.filter(c => c.g === g).map(c => (
                  <button key={c.id} onClick={() => { setRcptCatF(c.id); setShowFilterPick(false); setShowVault(true); }} className={`w-full text-left px-4 py-3 rounded-2xl text-xs font-bold flex items-center justify-between transition mb-1 ${rcptCatF === c.id ? "bg-blue-50 text-blue-800 border border-blue-200" : "text-gray-700 hover:bg-gray-50"}`}>
                    <div><p className="font-bold">{c.emoji} {c.name}</p><p className="text-[10px] text-gray-400 font-normal">Max RM {getCatLimit(c).toLocaleString()}</p></div>
                    {rcptCatF === c.id && <Check className="w-4 h-4 text-blue-600" />}
                  </button>
                ))}
              </div>
            ))}
