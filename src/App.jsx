...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-950 via-slate-900 to-emerald-950 font-sans flex flex-col">
      {toast && (
        ...
        <div className="bg-gray-900 text-white text-xs font-bold py-2.5 px-4 flex items-center justify-center gap-3 flex-wrap">
          ...
          {isIOS ? (
            ...
            <>
              ...
              <button onClick={handleInstallClick} className="px-3 py-1 bg-gradient-to-r from-emerald-700 to-teal-700 hover:from-emerald-600 hover:to-teal-600 rounded-lg">Install</button>
            </>
          )}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-900 border border-emerald-300">YA {taxYear}</span>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-4 space-y-4 pb-24">
        ...
        {/* Hero Banner */}
        <div className="bg-gradient-to-br from-emerald-950 via-emerald-900 to-teal-950 backdrop-blur-md rounded-3xl p-5 sm:p-6 shadow-xl border-2 border-emerald-700/70 relative overflow-hidden space-y-4">
          <div className="absolute -top-10 -right-10 w-32 h-32 bg-emerald-700/20 rounded-full blur-2xl pointer-events-none" />
          <div className="absolute -bottom-10 -left-10 w-32 h-32 bg-teal-500/20 rounded-full blur-2xl pointer-events-none" />

          <div className="border-b border-emerald-700/70 pb-4 relative">
            <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white">Salam{clientName ? `, ${clientName}` : ""} 👋</h1>
            <p className="text-xs text-emerald-200/90 font-semibold">Track YA {taxYear} reliefs · Log receipts · Maximize your refund before 31 Dec {taxYear}</p>
          </div>
          <div className="grid grid-cols-2 gap-3 relative">
            <div className={`bg-gradient-to-br ${myBalance <= 0 ? "from-emerald-800 to-teal-800 border-emerald-600" : "from-amber-50 to-orange-50 border-amber-200"} border p-3 rounded-2xl`}>
              <p className={`text-[10px] font-black uppercase tracking-wider ${myBalance <= 0 ? "text-emerald-100" : "text-amber-600"}`}>{myBalance <= 0 ? "Est. Refund" : "Est. Balance Owed"}</p>
              <p className={`text-lg sm:text-2xl font-black mt-0.5 ${myBalance <= 0 ? "text-emerald-100" : "text-amber-600"}`}>{fmt(Math.abs(myBalance), 0)}</p>
              <p className={`text-[10px] font-medium opacity-70 ${myBalance <= 0 ? "text-emerald-100" : "text-amber-600"}`}>vs PCB already paid</p>
            </div>
            <div className="bg-gradient-to-br from-emerald-800 to-teal-800 border border-emerald-600 p-3 rounded-2xl">
              <p className="text-[10px] font-black uppercase tracking-wider text-emerald-100">Total Reliefs Claimed</p>
              <p className="text-lg sm:text-2xl font-black mt-0.5 text-white">RM {totalReliefs.toLocaleString()}</p>
              <p className="text-[10px] font-medium opacity-80 text-emerald-100">Chargeable income: RM {chargeable.toLocaleString()}</p>
            </div>
          </div>
        </div>
                ...
                <div className="space-y-2.5">
                  {opps.slice(0, 4).map(c => (
                    <div key={c.id} className="p-3 rounded-2xl bg-gray-50 border border-gray-100 hover:border-emerald-200 transition space-y-1.5">
                      ...
                      {c.isAuto ? (
                        <button onClick={() => openSettings("income")} className="w-full py-1.5 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white font-bold text-[11px] border border-emerald-200 flex items-center justify-center gap-1">Edit in Profile</button>
                      ) : (
                        <button onClick={() => { setForm({ ...blank(taxYear), category: c.id }); setShowReceipt(true); }} className="w-full py-1.5 rounded-xl bg-emerald-800 hover:bg-emerald-700 text-white font-bold text-[11px] border border-emerald-200 flex items-center justify-center gap-1"><Plus className="w-3 h-3" /> Add Receipt</button>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {(() => {
        ...
        const tabBtn = (active, onClick, Icon, label) => (
          <button onClick={onClick} className={`flex flex-col items-center gap-0.5 px-4 py-1.5 rounded-2xl transition ${active ? "text-emerald-600" : "text-gray-400"}`}>
      ...
      {showTools && (
        <div className="fixed inset-0 z-[65] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => setShowTools(false)}>
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-gradient-to-br from-emerald-950 via-slate-900 to-teal-950 rounded-3xl p-5 text-white shadow-md space-y-3">
              ...
              <a href="https://mytax.hasil.gov.my" target="_blank" rel="noreferrer" className="w-full py-2 bg-gradient-to-r from-emerald-800 to-teal-800 hover:from-emerald-700 hover:to-teal-700 text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition"><ExternalLink className="w-3.5 h-3.5" /> MyTax Official Portal</a>
            </div>
      ...
      {showAuditCheck && (
        <div className="fixed inset-0 z-[70] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => { setShowAuditCheck(false); setShowTools(true); }}>
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-emerald-800 border border-emerald-600 text-white rounded-2xl p-4 text-center space-y-1">
            ...
            <button onClick={exportPDF} className="w-full py-3 bg-gradient-to-r from-emerald-700 to-teal-700 hover:from-emerald-600 hover:to-teal-600 text-white font-extrabold rounded-2xl text-xs flex items-center justify-center gap-2 shadow">
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              {scenarioCalc.isConnected ? (
                <div className="p-3.5 bg-emerald-800 border border-emerald-600 text-white rounded-2xl space-y-2">
        ...
        <div className="fixed inset-0 z-[90] bg-gray-900/70 backdrop-blur-sm overflow-y-auto p-4" onClick={closePaywall}>
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 space-y-4">
              ...
              {!trialUsed ? (
                <div className="space-y-2">
                  <button onClick={startTrial} disabled={paymentLoading} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-700 to-teal-700 hover:from-emerald-600 hover:to-teal-600 disabled:opacity-50 text-white font-extrabold transition shadow-md">Start 7-Day Free Trial</button>
                ...
                <button onClick={subscribe} disabled={paymentLoading} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-700 to-teal-700 hover:from-emerald-600 hover:to-teal-600 disabled:opacity-50 text-white font-extrabold transition shadow-md">{paymentLoading ? "Redirecting to payment…" : `Subscribe to Plus — RM${PRICE.toFixed(2)}/yr`}</button>
              )}
              {isTrialing ? (
        ...
        <div className="fixed inset-0 z-[70] bg-gray-900/60 backdrop-blur-sm overflow-y-auto p-4" onClick={() => { setShowFormBE(false); setShowTools(true); }}>
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl mx-auto my-8 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50">
              <div className="flex items-center gap-2.5">
                <div className="p-2.5 bg-emerald-800 text-white rounded-2xl shrink-0"><Sparkles className="w-5 h-5" /></div>
                <div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className={`rounded-2xl border overflow-hidden ${spouseAnalysisDetailed.recommended === "SEPARATE" ? "border-emerald-300 ring-1 ring-emerald-200" : "border-gray-200 opacity-90"}`}>
                            <div className="px-3 py-2 bg-gray-900 text-white flex items-center justify-between">
                              ...
                              {spouseAnalysisDetailed.recommended === "SEPARATE" && <span className="text-[9px] bg-emerald-700 px-2 py-0.5 rounded-full font-bold">Recommended</span>}
                            </div>
                          ...
                          <div className={`rounded-2xl border overflow-hidden ${spouseAnalysisDetailed.recommended === "JOINT" ? "border-emerald-300 ring-1 ring-emerald-200" : "border-gray-200 opacity-90"}`}>
                            <div className="px-3 py-2 bg-gray-900 text-white flex items-center justify-between">
                              ...
                              {spouseAnalysisDetailed.recommended === "JOINT" && <span className="text-[9px] bg-emerald-700 px-2 py-0.5 rounded-full font-bold">Recommended</span>}
                            </div>
                            ...
                            {spouseAnalysisDetailed.catBreakdown.filter(c => c.mine > 0 || c.spouse > 0).map(c => (
                              <div key={c.id} className="space-y-1">
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <div className="flex-1 h-1.5 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-emerald-700" style={{ width: `${Math.min(100, (c.mine / c.limit) * 100)}%` }} /></div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            {settingsTab === "income" && (
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-2xl bg-emerald-800 border border-emerald-600 text-white space-y-2 text-xs">
              ...
              <div className="space-y-4">
                {!hasSpouse ? (
                  ...
                  <>
                    ...
                    <div className="p-4 rounded-2xl bg-emerald-800 border border-emerald-600 text-white space-y-3 text-xs">
                    ...
                    <div className="border-t border-gray-100 pt-4">
                      <button onClick={() => { setShowSettings(false); setShowSpouseDetail(true); }} className="w-full py-2.5 rounded-xl bg-emerald-50 hover:bg-emerald-800 text-white font-bold text-xs border border-emerald-200 flex items-center justify-center gap-1.5">
            ...
            <button onClick={saveAll} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-700 to-teal-700 hover:from-emerald-600 hover:to-teal-600 text-white font-extrabold transition shadow-md">Save Settings</button>
          </div>
        </div>
      )}
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-emerald-200 rounded-2xl p-6 text-center space-y-3 bg-emerald-50">
                  ...
                  <label className="cursor-pointer inline-block">
                    <div className="px-6 py-2.5 rounded-xl bg-emerald-600 text-white font-bold text-xs hover:bg-emerald-700 transition inline-flex items-center gap-2"><Upload className="w-4 h-4" /> Choose Receipt Photo</div>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl space-y-4 mx-auto my-8" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={editId ? handleUpdate : handleSave} className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-700">{editId ? "Update" : "Vault Receipt"}</button>
              </div>
            </div>
          </div>
        </div>
